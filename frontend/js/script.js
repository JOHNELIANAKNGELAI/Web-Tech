...
window.switchModule = function(index) {
  ...
  if (index === 0) {
    let vSrc = 'videos/cs_intro.mp4'; // Default fallback
    if (course && course.title) {
      ...
      else if (course.video_url) vSrc = course.video_url;
    }
    console.log("Loading video source:", vSrc);
    ...
    container.innerHTML = `
      ...
      <div style="width: 100%; background: #0F172A; border-radius: 12px; margin-bottom: 24px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.15);">
        <video controls preload="auto" style="width: 100%; display: block; max-height: 450px;">
          <source src="${vSrc}?v=${new Date().getTime()}" type="video/mp4">
